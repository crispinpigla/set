        ...
        try:
            ...
            if connexion_form.is_valid():
            	...
            	if len(users_in_database) == 0:
            		return HttpResponse("Aucun utilisateur ne possede un compte avec cette adresse mail")
            	else:
    ...
    try:
        ...
        try:
            ...
            if inscription_form.is_valid() and (request.POST["password"] == request.POST["confirmation_password"]):
            	users_in_database = Utilisateurs.objects.filter(adresse_mail=request.POST["email"])
            	if len(users_in_database) == 0:
            		user = Utilisateurs.objects.create(nom=request.POST["name"], adresse_mail=request.POST["email"], mot_de_passe=request.POST["password"],)
            		request.session["user_id"] = user.id
            		return redirect("../../user/home/")
            	else:
            		return HttpResponse("Un compte possedant cette adresse mail existe deja")
            else:

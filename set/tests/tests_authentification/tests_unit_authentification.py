...
class AuthentificationViewsTestCase(TestCase):
	...
	def setUp(self):
		...
		self.client_registred = Client()

		self.user_registred = Utilisateurs.objects.create(nom="user_registred", adresse_mail="user_registred@mail.mail", mot_de_passe='user_registred_password')

		self.client_registred.post('/authentification/connexion/', {'email':'user_registred@mail.mail', 'password':'user_registred_password'})


	def test_connexion_0(self):
		""""""
		#  Connexion reussie
		response = self.client_not_registred.post('/authentification/connexion/', {'email':'mail@mail.mail', 'password':'password'})
